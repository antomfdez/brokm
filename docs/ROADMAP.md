...
- **Path:** lexer ✅ → parser/AST ✅ → code generator ✅ → next, target the C VM's *real* opcode
  format (so brokm-generated bytecode runs on the C VM directly), then — much later — a runtime in
