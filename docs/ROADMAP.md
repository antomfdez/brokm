...
- **Three-stage fixpoint:** have the self-compiled realcc re-compile realcc and diff the
  ...
  (`make test-bootstrap` today only checks two stages agree on `sample.bk`).
- **`Assemble` constant guard:** realcc's constant pool dedups but does not bound; a function
  with >256 *unique* constants would silently wrap its one-byte operand again (the step-22 bug
  class). The C compiler errors — `Assemble` should too.
